...
// Move sphere up on mobile to avoid covering text
if (isMobile) {
    sphere.position.y = 4;
}
...
// Cross-Tab Logic (Only for Desktop)
const bc = !isMobile ? new BroadcastChannel('portfolio_sync') : null;
...
if (bc) {
    ...
    setInterval(() => {
        bc.postMessage(getScreenPos());
    }, 100);
}

const boltMat = new THREE.LineBasicMaterial({ color: 0x00f2ff, transparent: true, opacity: 1 });

const createBoltPoints = (dx, dy, segments = 15) => {
    const points = [];
    points.push(new THREE.Vector3(0, isMobile ? 4 : 0, 0));
    for (let i = 1; i < segments; i++) {
        const t = i / segments;
        const jitter = 1;
        points.push(new THREE.Vector3(
            dx * t + (Math.random() - 0.5) * jitter,
            (isMobile ? 4 : 0) + dy * t + (Math.random() - 0.5) * jitter,
